...
func TestTrafficMatrix(t *testing.T) {
	...
	cases := []struct {
		...
		{"cloudflare", "cloudflare.com", decision.ModeDirect, "DefaultMode=DIRECT (unknown → ISP)"},
		{"ifconfig", "ifconfig.me", decision.ModeDirect, "exit-IP check shows ISP when unknown"},
		{"ipify", "api.ipify.org", decision.ModeDirect, "exit-IP check shows ISP when unknown"},
